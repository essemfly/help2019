...
class ConvConvConv(nn.Module):
    def __init__(self, prior_prob=None):
        ...
        self.embedding = nn.Sequential(
            nn.Conv1d(model_config['measure_dim'], model_config['embedd_dim'], kernel_size=1),
            nn.BatchNorm1d(model_config['embedd_dim']))
        self.block1 = nn.Sequential(
            nn.Conv1d(model_config['embedd_dim'], 2 * model_config['embedd_dim'], kernel_size=1),
            nn.BatchNorm1d(2 * model_config['embedd_dim']), nn.ReLU())
        self.block2 = nn.Sequential(
            nn.Conv1d(2 * model_config['embedd_dim'], model_config['embedd_dim'], kernel_size=1),
            nn.BatchNorm1d(model_config['embedd_dim']), nn.ReLU())
        self.pooling = nn.AvgPool1d(hyperparams['max_seq_len'])
        self.linear1 = nn.Sequential(nn.Linear(model_config['embedd_dim'], model_config['ffn_dim']),
                                     nn.BatchNorm1d(model_config['ffn_dim']), nn.ReLU())
